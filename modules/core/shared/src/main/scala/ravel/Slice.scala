...
object Slice:
  def apply(start: Int, stopExclusive: Int, step: Int = 1): Slice =
    from(start, stopExclusive, step).fold(
  ...
  def from(
      ...
      stopExclusive: Int,
      step: Int = 1
  ): Either[SliceError, Slice] =
    if step == 0 then Left(SliceError("step must not be zero"))
    else Right(new Slice(start, stopExclusive, step, false, false))

  def from(range: Range): Either[SliceError, Slice] =
    val stop =
      if range.isInclusive then
        ...
        if shifted < Int.MinValue.toLong || shifted > Int.MaxValue.toLong then
          return Left(SliceError("inclusive range endpoint cannot be represented exclusively"))
        shifted.toInt
      else range.end
