...
| Reusable law-test module | Internal | `ravel-laws` has no main-source classes or published coordinate; its generated checks are repository-only. | A future public kit requires a new adapter design and release line. |
| Test framework and Discipline integration | Strong | Internal MUnit and ScalaCheck checks run on both platforms. `InternalRavelDiscipline` contains generated rather than constant properties and executes with deterministic single-worker parameters. | No downstream RuleSet or compatibility promise exists. |
